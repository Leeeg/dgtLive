...
public class CropImageView extends ImageView {
    ...
    private int mBorderLineWidth = 4;         //虚线框的宽度（画笔宽度）
    private int mFocusWidth = getWidth();         //焦点框的宽度
    private int mFocusHeight = getHeight();        //焦点框的高度
    private int mDefaultStyleIndex = 0;    //默认焦点框的形状
    private final  String NOTIFY = "虚线内为电脑端展示区域";
    private float textStart, textEnd;
    ...
    private void initImage() {
        ...
        mLineRect.left = mFocusMidPoint.x - mFocusWidth / 2;
        mLineRect.right = mFocusMidPoint.x + mFocusWidth / 2;
        mLineRect.top = mFocusMidPoint.y - mFocusHeight / 2 + 74;
        mLineRect.bottom = mFocusMidPoint.y + mFocusHeight / 5 - 74;

        mTxtPaint.setColor(Color.WHITE);
        mTxtPaint.setTextSize(34.0f);// 字体大小
        mTxtPaint.setTypeface(Typeface.DEFAULT_BOLD);// 采用默认的宽度
        mTxtPaint.setShadowLayer(0, 2, 2, 000000);// 设定阴影(柔边, X 轴位移, Y 轴位移, 阴影颜色)
        float textW = mTxtPaint.measureText(NOTIFY);
        textStart = mFocusMidPoint.x - (textW/2);
        textEnd = mFocusMidPoint.y + mFocusHeight / 5 - 84;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (Style.RECTANGLE == mStyle) {
            ...
            mLinePath.addRect(mLineRect, Path.Direction.CCW);
            canvas.save();
        ...
        canvas.drawText(NOTIFY, textStart, textEnd, mTxtPaint);

        mLinePaint.setPathEffect(new DashPathEffect(new float[]{15, 5}, 0));
        mLinePaint.setStyle(Paint.Style.STROKE);
        mLinePaint.setStrokeWidth(mBorderLineWidth);
